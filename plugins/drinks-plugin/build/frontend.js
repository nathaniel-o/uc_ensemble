...
function initLightbox() {
  ...
  document.addEventListener('touchstart', handleLightboxTouch, {
    passive: true
  });
...
function ucSummonCarousel(context) {
  ...
  loadCarouselImages(overlay, context.matchTerm || '', context.filterTerm || '', context.container || null);
  ...
  if (slideshowContainer && slideshowContainer.swiper) {
    ...
    if (swiper.params.pagination && swiper.params.pagination.el) {
      ...
      const updatePaginationDisplay = () => {
        ...
        const nonDuplicateSlides = Array.from(swiper.slides).filter(slide => !slide.classList.contains('swiper-slide-duplicate'));
      ...
      swiper.params.pagination.renderCustom = function (swiperInstance, current, total) {
        // Count only non-duplicate slides
        const nonDuplicateSlides = Array.from(swiperInstance.slides).filter(slide => !slide.classList.contains('swiper-slide-duplicate'));
        const realSlidesCount = nonDuplicateSlides.length;
...
function ucPortraitLandscape(imageElement) {
  ...
  if (container.classList.contains('pop-off') || container.classList.contains('wp-block-gallery') || container.classList.contains('portrait') || container.classList.contains('landscape')) {
...
function initSearchPageCarousel() {
  ...
  const isSearchPage = document.body.classList.contains('search') || document.body.classList.contains('search-results');
  ...
  ucSummonCarousel({
    ...
    container: null,
    isOverlay: false,
    // Inline mode - allow page scrolling
    closePopOut: false,
    // No pop-out on page load
